...
while totalcount < N: #for each sample
    pos_R = np.random.uniform(-R,R,size=(n,1)) #generates an array of random numbers within -R and R
    total = 0 #sets total to 0 for each loop
    
    total = sum(pos_R**2) #gives the total of x^2+y^2+...
    sigmarr=np.append(sigmarr,np.sqrt(sum(pos_R**2)))#adds the square root of the total to an array
        
    
    r = np.sqrt(total) #square rooting over the sum of all random numbers squared
    if r <= R: #tests if the number is within the sphere
        circlecount += 1
    totalcount += 1
mean=sum(sigmarr)/N #sums the radii of points, divides by number of samples for the mean
for i in range(N): #does (xi-xm)^2 for all values
