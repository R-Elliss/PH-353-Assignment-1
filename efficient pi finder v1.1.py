...
while totalcount < N:
    pos_R = np.random.uniform(-R,R,size=(n,1))
    radsq = sum(pos_R**2) 
    sigmarr=np.append(sigmarr,np.sqrt(radsq))
    #total += np.sqrt(radsq)
    
    r = np.sqrt(radsq) #square rooting over the sum of all random numbers squared
